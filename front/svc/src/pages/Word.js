...
const WordForm = (props) => {
    ...
    const columns = [
        ...
        { field: 'word_name', headerName: 'Word Name', width: 300, },
    ...
    const WORD_LIST_URL = "/api/db/select/wordList";
    const [boardList, setBoardList] = useState([]);

    const fetchList = useCallback(async () => { 
        try {
            //console.log("fetchList" + props.inputs.word);
            setLoadingYn(true);
            axios(WORD_LIST_URL, {
                method: 'GET',
                headers: {
                ...
                params: { 
                    ...
                    word: props.inputs.word,
                    word_name: props.inputs.wordName,
                },
            })
            .then((res) => {
    ...
    // Function called when the word, wordname changed
    useEffect(() => {
        //console.log("WordForm: useEffect" + props.inputs.word);
        ...
        <Box sx={{ width: '100%' }}>
            <Box sx={{ display: 'flex', m: 1, mt: 2}}>
                ...
                <TextField name="word"
                           ...
                           focused
                />
                <TextField name="wordName"
                           value={inputs.wordName}
                           label="Word Name"
                           onChange={handleChange}
                           size="small"
                           inputProps={{style: {fontSize: 14}}}
            ...
            <Box sx={{ height: '77vh', width: '78vw', m: 1, mt: 2}}>
                <WordForm dbName={dbName} dbUser={dbUser} inputs={inputs} />
            </Box>
